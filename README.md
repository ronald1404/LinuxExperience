...
### Diretórios
    - mkdir nomedapasta (criar o diretório)
    - rmdir nomedapasta (exclui a pasta vazia)
...
### Criando e excluindo usuários:
    ...
    - userdel -r -f nome do usuário (remove, force)
...
### Criando Novos Grupos
    ...
    - cat/etc/group (Verificar os grupos)
...
### Alterando as permissões de um diretório-arquivo
    ...
    - chown usuario:grupo PastaOuArquivo (alterando dono e grupo do item)

    - chmod xxx(Para mudar as permissões para que o dono, usuários do grupo, e os de fora possam ser modificados)
    somar os números abaixo para modificar permissões
    ...
    EX: 
    fazer com que o dono possa ler, gravar e executar = 4+2+1 = 7
    usuários do grupo possam ler e executar = 4+1 = 5
    outros não possam fazer nada = 0

    - chmod 750 NomeDaPastaOuArquivo

    - chown nomeUser:nomeGrupo NomeDaPastaOuArquivo (para alterar dono e grupo)
